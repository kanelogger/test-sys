...
- 保留 9/8 与 9/6 备份版本兼容，初始化仍不自动覆盖。线上浏览器检查到 139 项均 pending、0 条日志，调整前快照已保存本机临时目录；不代表其他浏览器无记录。
- 本轮静态校验见 `docs/seed-validation.json`；应用验证结果在本轮完成后补充。旧模型推演与 final-acceptance 证据只对应 9/8 版本，不宣称已验证新版。
