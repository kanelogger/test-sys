# 当前调整：2026-09-20 按实际学习反馈滚动修订

- 本轮起始 SHA：`ed2333896cd619aa1c47e9acc8e1d566e9235f49`。用户报告导学实际 14 分钟、无卡点，要求后续随实际学习情况调整。
- 唯一种子升级为 `sysanalyst-2026-09-20.v2`：34 天、101 项、3925 分钟（65 小时 25 分钟），预算 4260，缓冲 335；兼容 v1、9/8、9/6 备份。初始化不自动覆盖已有浏览器数据。
- 用户提供 `公共课.json`，56 条、51168 秒。依据原速时长细化最近 3 天：9/20 软件工程 60 分钟（视频 51:55）；9/21 需求获取/验证/管理 45 分钟、结构化分析 20 分钟；9/22 函数依赖/规范化 45 分钟。9/23 起保留暂定安排，以实际分钟、停止位置、闭卷产出、卡点/错题继续修订。
- 真实本地学习助手已通过计划页同步 4 项修订：旧 pending 跳过保留，新建修订项并排序。9/21 与 9/22 的有效任务各 90 分钟。刷新后今日显示 115/90 分钟（超通用参考线 25 分钟，仍在周日 240 分钟预算内），导学已完成且实际 14 分钟，其余预计 100 分钟；未清空或覆盖学习日志。
- 本地页面 `http://127.0.0.1:5173/test-sys/#/` 已停留在今日，可从软件工程开始。线上部署尚未更新。真实页面证据来自 UI 文本与操作反馈，本轮未保存新截图，也未重新执行全部生产验收。
- 类型检查、生产构建、完整浏览器测试 12 文件 84 项通过。提交后以本轮固定起点执行双轴评审；结果由任务最终回复报告。
- 当前成果是计划调整与页面可学习；通过考试仍需用户真实学习、训练表现和考试成绩，不虚构完成记录。

---

# 历史调整：2026-09-20 从第一天开始

- 用户已授权按今天重排；沿用考试日 10/24、星期预算与既有功能范围。
- 起始 SHA：`33332e02f49e94b3747501c663735724eb8454b4`。
- 唯一种子仍为 `public/data/study-plan.seed.json`，版本 `sysanalyst-2026-09-20.v1`；覆盖 9/20—10/23，34 天、101 项、3900 分钟，预算 4260，缓冲 360。
- 4 次年度题组（第 4 次分段）、6 个案例主题、6 份论文提纲、2 篇全文及修改、三科模拟。9/20 先做 90 分钟入门，150 分钟留作缓冲。
- 保留 9/8 与 9/6 备份版本兼容，初始化仍不自动覆盖。线上浏览器检查到 139 项均 pending、0 条日志，调整前快照已另存本机 `study-backup-before-20260920.json`（Git 忽略）；不代表其他浏览器无记录。
- 本轮静态校验见 `docs/seed-validation.json`；完整浏览器测试 12 文件 83 项、类型检查与生产构建通过。真实本地页面首次打开和刷新均为 9/20、距考试 34 天、5 项/90 分钟、无逾期；101 项 pending、0 日志。页面证据见 `docs/evidence/replan-20260920/`。本地页面 `http://127.0.0.1:5173/test-sys/` 已打开，线上部署尚未更新。旧模型推演与 final-acceptance 证据只对应 9/8 版本，不宣称已验证新版。
- 实现提交 `db851ac` 已相对本轮起点完成 code-review：Standards 0、Spec 0。后续文档提交仅补录运行事实。
- 通过考试仍需真实学习和考试成绩；没有填入虚构学习记录。

---

以下保留 2026-09-08 实施阶段历史证据，日期、任务数量和 SHA 均只适用于当时版本。
