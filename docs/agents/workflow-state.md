...
- COV-01（**已于 2026-09-07 经用户确认定案**）：`coverage.startDate/endDate` 字段命名、种子顶层位置及含首尾表示维持本节约定，作为已确认生产口径；替代命名不得在无记录情况下悄悄切换。
...
- COV-01：**已定案（2026-09-07 用户确认）**，见「覆盖表示约定」与「基线与当前阶段」冻结记录。
...
- 分发边界：React/Vite 应用脚手架已建立（T-00，2026-09-07），`public/data/` 种子随构建分发且子路径可加载已验证（dev 与 preview）；初始化接入（T-02）与 GitHub Pages 部署（T-06）未验证。
- 冻结门槛：真实种子校验与四场景模型推演已具备证据；COV-01 已于 2026-09-07 确认，决定依据与冻结时间已记录于「基线与当前阶段」，v2.6 功能范围已冻结（H-00 放行）。生产运行验收仍单列为未完成。
...
- 待决项归属不变：COV-01 阻塞需求冻结、不阻塞拆票，但 COV-01 决定与冻结时间是 T-02 起业务实施的硬前置（见 TRACK-02）；FMT-01 为 T-06 实施前置；PROTO-01 四项契约修订候选为 T-04 实施前置；各票公开契约随票细化（FLOW-01-L）。
- 需求仍未冻结（`frozenAt = null`）；本发布不替代 H-00 冻结放行与生产运行验收。
