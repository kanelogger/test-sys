...
> 基线版本：v2.6，2026-09-07；初始计划内容于 2026-09-20 按最新总计划重排。本文整体取代 v1.x 系列及 v2.0–v2.5。纯静态、纯本地应用：无后端、无登录、无数据库服务。
...
**初始计划内容要求**：种子须声明覆盖起止日，与最新备考执行窗口一致（2026-09-20 至 2026-10-23，考试前一日），逐日给出安排。任务按"一次学习时段内可完成"拆分；每项 `plannedMinutes` 为正整数，同一天所有初始任务的 `plannedMinutes` 总和不得超过当日预算（周一至周四 90 分钟、周五 60 分钟、周六日各 240 分钟）；每个任务填写非空 `completionCriteria` 并尽量关联 `resourceId`（网页资源给可跳转 http(s) URL，本地 PDF 只给文件名），使当天任务直接回答"从哪里开始"。种子内容以本文"已报名、已缴费、零进度"的前提为准：《系统分析师备考总计划》2026-09-20 按用户要求以 9/20 为第一天，9/1—9/19 旧日程退出执行表，因此这些日期不进入种子，也不得成为逾期欠账。
